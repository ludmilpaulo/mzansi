...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  try {
    ...
    if (landing.kind !== "location") {
      return { title: "Not found", robots: { index: false, follow: false } };
    }
    return metadataFromSeoFields(
      landing,
      { title: landing.title, description: landing.excerpt, path: `/locations/${slug}` },
      defaults,
    );
  } catch {
    return { title: "Location", robots: { index: false, follow: false } };
  }
}

export default async function LocationLandingPage({ params }: Params) {
  const { slug } = await params;
  ...
  if (!landing || landing.kind !== "location") {
    if (errorMessage?.toLowerCase().includes("not found") || (landing && landing.kind !== "location")) {
      notFound();
    }
    return (
